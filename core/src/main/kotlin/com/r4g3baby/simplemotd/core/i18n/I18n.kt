...
class I18n(language: String = "en", private val plugin: MOTDPlugin) : I18n {
        ...
        val translation = translateHexColorCodes(
            translateAlternateColorCodes(
                prefix + try {
                    ...
                    plugin.getLogger().log(Level.WARNING, "Missing translation key \"$key\".", ex)
                    key
                }
            )
        )
    ...
    private class PluginResClassLoader(plugin: MOTDPlugin) : ClassLoader(plugin.javaClass.classLoader) {
